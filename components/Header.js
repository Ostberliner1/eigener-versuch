  ...
  return (
    <header className="bg-gradient-to-r from-black via-red-600 to-yellow-500 text-white p-4">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <nav className={`
          md:flex md:items-center md:space-x-6 text-xl
          ${isOpen ? 'block absolute top-16 left-0 w-full bg-black p-4' : 'hidden'}
        `}>
          <Link href="/shop" className="block md:inline-block mt-2 md:mt-0 hover:text-blue-300">
            Shop
          </Link>
          <Link href="/kontakt" className="block md:inline-block mt-2 md:mt-0 hover:text-blue-300">
            Kontakt
          </Link>
          <Link href="/musik" className="block md:inline-block mt-2 md:mt-0 hover:text-blue-300">
            Musik
          </Link>
          <Link href="/info" className="block md:inline-block mt-2 md:mt-0 hover:text-blue-300">
            Info
          </Link>
        </nav>
      </div>
